...
class SoundPlayListRepositoryTest {
    ...
    @Test()
    fun ` findAllPlayListWithSong should return a emptyList`() =
        runTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `When findAllPlayListWithSong method is executed,it should return a list of PlaylistWithSoundDomain`() =
        runTest {
            val mockPlayListWithSongList =
                listOf(
                    PlayListWithSong(playList = playList.toEntity(), soundOfPlayList = soundEntityList),
                )

            Mockito
                .`when`(playlistAndSoundCrossDao.findAllPlayListWithSong())
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `Given a valid playlist, when savePlaylist runs, then returns inserted ids`() =
        runTest {
            Mockito.`when`(playListDAO.createPlayList(playList.toEntity())).thenReturn(1L)
            Mockito
                .`when`(
                    playlistAndSoundCrossDao.insertPlayListAndSoundCross(Mockito.anyList()),
                ).thenReturn(listOf(1, 2))

            val result = soundPlayListRepository.savePlayList(playList)

            assertThat(result).isEqualTo(listOf<Long>(1, 2))
            verify(playlistAndSoundCrossDao, times(1)).insertPlayListAndSoundCross(Mockito.anyList())
    ...
    fun `savePlayList should launch NullPointerException when invalid id`() =
        runTest {
            Mockito.`when`(playListDAO.createPlayList(playList.toEntity())).thenReturn(1L)
            Mockito
                .`when`(
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `findPlayListById should find playList by your id`() =
        runTest {
            val playListWithSong =
                PlayListWithSong(
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test(expected = RepositoryException::class)
    fun `findPlayListById should launch NullPointerException when invalid id`() =
        runTest {
            ...
            try {
                ...
                assertThat(e.message).isEqualTo("Erro ao buscar playlist, ID inválido")
                assertThat(e).isInstanceOf(RepositoryException::class.java)
                verify(playListDAO, times(1)).findPlayListById(Mockito.anyLong())
                throw e
            }
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `deletePlaylist Should delete playlist `() =
        runTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test(expected = RepositoryException::class)
    fun `deletePlaylist should launch NullPointerException when id notFound or invalid`() =
        runTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `addSountToPlayList should add sound in playlist`() =
        runTest {
            Mockito.`when`(playlistAndSoundCrossDao.insertPlayListAndSoundCross(Mockito.anyList())).thenReturn(
                listOf(1, 2),
            )
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test(expected = RepositoryException::class)
    fun `addSountToPlayList should launch NullPointerException when id is invalid`() =
        runTest {
            Mockito
                .`when`(
                    playlistAndSoundCrossDao.insertPlayListAndSoundCross(Mockito.anyList()),
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `removeSoundItemFromPlayList should remove Sound in to PlayList`() =
        runTest {
            Mockito
                .`when`(
                    playlistAndSoundCrossDao.deleteItemPlayListAndSoundCross(Mockito.anyLong(), Mockito.anyLong()),
                ).thenReturn(1)
            ...
            verify(
                ...
                times(1),
            ).deleteItemPlayListAndSoundCross(Mockito.anyLong(), Mockito.anyLong())
        }

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test(expected = RepositoryException::class)
    fun `removeSoundItemFromPlayList should lauch NullPointerException when id invalid`() =
        runTest {
            Mockito
                .`when`(
                    playlistAndSoundCrossDao.deleteItemPlayListAndSoundCross(Mockito.anyLong(), Mockito.anyLong()),
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `updateNamePlayList should update name of the playlist `() =
        runTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test(expected = RepositoryException::class)
    fun `updateNamePlayList should launch NullPointer if id invalid`() =
        runTest {
            Mockito
                .`when`(
                    playListDAO.updateNamePlayList(Mockito.anyLong(), Mockito.anyString()),
                ).thenThrow(NullPointerException("erro id"))
