...
class PlayListDAOTest {
    ...
    fun setUp() {
        ...
        playlistAndSoundCrossDao = databasePlayList.playListAndSoundCrossDao()
    }

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun given_a_valid_playList_when_createPlayList_is_executed_it_should_save_a_play_list_and_return_long_value() =
        runTest {
            val playListEntity =
                PlayListEntity(
                    playListId = null,
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun when_findAllPlayList_is_executed_should_return_a_list_of_playListWithSounds() =
        runTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun given_a_valid_playList_when_updatePlaylist_is_executed_should_update_data_in_playlist() =
        runTest {
            HelperDao.listOfPlayList().forEach { playListDAO.createPlayList(it) }
            val playListToUpdate =
                PlayListEntity(
                    playListId = 4,
    ...
    @Test
    fun given_a_valid_string_and_id_when_is_executed_updateNamePlayList_it_should_update_name_of_playlist() =
        runTest {
            HelperDao.listOfPlayList().forEach { playListDAO.createPlayList(it) }
            val result = playListDAO.updateNamePlayList(id = 3, "Nome Atualizado")
            assertThat(result).isEqualTo(1)
    ...
    @Test
    fun given_a_string_and_invalid_id_when_is_executed_updateNamePlayList_it_should_return_zero() =
        runTest {
            HelperDao.listOfPlayList().forEach { playListDAO.createPlayList(it) }
    ...
    @Test
    fun given_a_id_valid_when_findPlayListById_is_executed_it_should_return_a_PlayListWithSong() =
        runTest {
            HelperDao.listOfPlayList().forEach { playListDAO.createPlayList(it) }
    ...
    @Test
    fun given_playlist_when_deletePlayList_is_executed_should_delete_playlist_and_return_affected_rows() =
        runTest {
            HelperDao.listOfPlayList().forEach { playListDAO.createPlayList(it) }
            ...
            val allPlayList = playlistAndSoundCrossDao.findAllPlayListWithSong()
